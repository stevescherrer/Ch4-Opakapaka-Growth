...
model_1 = jags(data, inits, 
               ...
               parameters.to.save =  c('Linf_mu', 'Linf_std', 'Linf_tau', 'Shape', 'k_mu', 'k_std', 'k_tau', 'rate', 'tau', 'variance', 'p.value'),
               DIC = T, 
               n.chains = 3, n.iter = 1550000, n.burnin = 150000, n.thin = 50)
# save.image(file.path(results_dir, 'Bayes/Bayesian Workspace.RData'))

# send_push(user = 'uGEHvA4hr37tsrCCtpSv4sUUxVuTqN', message = paste("Model 1 Fitting Complete - DIC =", model_1$BUGSoutput$DIC, 'Linf n.eff =', model_1$BUGSoutput$summary[1, 'n.eff']))
...
# send_push(user = 'uGEHvA4hr37tsrCCtpSv4sUUxVuTqN', message = paste("Model 2 Fitting Complete - DIC =", model_2$BUGSoutput$DIC))
...
# send_push(user = 'uGEHvA4hr37tsrCCtpSv4sUUxVuTqN', message = paste("Model 3 Fitting Complete - DIC =", model_3$BUGSoutput$DIC))
...
send_push(user = 'uGEHvA4hr37tsrCCtpSv4sUUxVuTqN', message = paste("Model 4 Fitting Complete - DIC =", model_4$BUGSoutput$DIC))

##### Model Diagnostics
### Calculate model P-Values by getting the mean value of all p.value entries in the model summary
model_1$p.value = mean(model_1$BUGSoutput$summary[grep('p.value', rownames(model_1$BUGSoutput$summary)), 'mean'])
model_2$p.value = mean(model_2$BUGSoutput$summary[grep('p.value', rownames(model_2$BUGSoutput$summary)), 'mean'])
model_3$p.value = mean(model_3$BUGSoutput$summary[grep('p.value', rownames(model_3$BUGSoutput$summary)), 'mean'])
model_4$p.value = mean(model_4$BUGSoutput$summary[grep('p.value', rownames(model_4$BUGSoutput$summary)), 'mean'])

range(model_1$p.value, model_2$p.value, model_3$p.value, model_4$p.value)

### Extract model DIC values
model_1$DIC = model_1$BUGSoutput$DIC
model_2$DIC = model_2$BUGSoutput$DIC
model_3$DIC = model_3$BUGSoutput$DIC
model_4$DIC = model_4$BUGSoutput$DIC
...
for(model in 1:length(growth_models)){
...
for(i in 1:length(growth_models)){
  ...
  cv_df = rbind(cv_df, data.frame('model' = names(growth_models)[i], 'Parameter' = 'Linf', 'Estimate' = curr_mod$BUGSoutput$summary["Linf_mu","mean"], 'StdDev' = curr_mod$BUGSoutput$summary["Linf_mu",'sd'], 'cv' = curr_mod$BUGSoutput$summary["Linf_mu",'sd'] / curr_mod$BUGSoutput$summary["Linf_mu","mean"] * 100), 
                data.frame('model' = names(growth_models)[i], 'Parameter' = 'k', 'Estimate' = curr_mod$BUGSoutput$summary["k_mu","mean"], 'StdDev' = curr_mod$BUGSoutput$summary["k_mu",'sd'], 'cv' = curr_mod$BUGSoutput$summary["k_mu",'sd'] / curr_mod$BUGSoutput$summary["k_mu","mean"] * 100))
}
