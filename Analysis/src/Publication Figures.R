...
plot(x = c(0, max_age), y = c(0, 100),
     xlim = c(0, max_age), xlab = 'Age (Years)',
     ylim = c(0, 80), ylab = "Fork Length (cm)",
     cex = 1.5, cex.axis = 1.5, cex.lab = 1.5, 
     type = 'n')

## Add extra tick marks
axis(1, at = seq(0, 20, 2.5), labels = F)
axis(2, at = seq(0, 80, 5), labels = F)
...
## Adding lines representing smallest and largest fish tagged at recapture
abline(min(tagdat[ ,2]), 0, lty = 2)
abline(max(tagdat[ ,2]), 0, lty = 2)
# text(x = 0.5, y = min(tagdat[ ,2]) + 5, labels = 'min Lr', cex = 1.5)
# text(x = 0.5, y = max(tagdat[ ,2]) + 5, labels = 'max Lr', cex = 1.5)
...
### Comparing 10000 iterations between integrative models
 ...
 models_to_predict_2 = lit_vbgc_params[lit_vbgc_params$author %in% c("Ralston and Miyamoto (1983) - Constrained Linf", "Ralston and Miyamoto (1983) - Unonstrained Linf", "DeMartini et al. (1994)", "Moffit and Parrish (1996) - Constrained Linf", "Andrews et. al (2012)", "OʻMalley (2015) - Gulland and Holt","OʻMalley (2015) - Faben"), ]
 models_to_predict_2$linf = as.numeric(models_to_predict_2$linf)
 models_to_predict_2$k = as.numeric(models_to_predict_2$k)
 models_to_predict_2$t0 = as.numeric(models_to_predict_2$t0)
 models_to_predict_2$t0[is.na(models_to_predict_2$t0)] = 0
 
 for(i in 1:nrow(models_to_predict_2)){
   predicted_recapture_lengths = rbind(predicted_recapture_lengths, predict_recapture_length(Lm = tagdat[ ,1], dt = tagdat[ ,4], linf = as.numeric(models_to_predict_2$linf[i]), k = as.numeric(models_to_predict_2$k[i])))
   predicted_recapture_residuals = rbind(predicted_recapture_residuals, predict_recapture_length(Lm = tagdat[ ,1], dt = tagdat[ ,4], linf = as.numeric(models_to_predict_2$linf[i]), k = as.numeric(models_to_predict_2$k[i])) - tagdat[ ,2])
 }
 
 rownames(predicted_recapture_lengths)   = models_to_predict_2$author
 rownames(predicted_recapture_residuals) = models_to_predict_2$author
 
 
 pdf(file.path(results_dir, 'IR Figure - Predicted vs. Observed LR Previous Studies.pdf'), height = 8.5, width = 8.5)
 par(mfrow = c(2, 4))
 
 predicted_recapture_lengths = 
 for(i in 1:nrow(predicted_recapture_lengths)){
   model_id = row.names(predicted_recapture_lengths)[i]
   plot(y = predicted_recapture_lengths[i, ], x = tagdat[ ,2],
        xlab = 'Observed Recapture FL (cm)', xlim = c(15, 80), 
        ylab = 'Predicted Recapture FL (cm)', ylim = c(15, 80),
        main = paste(model_id,'\n Linf = ', models_to_predict_2$linf[i],', K = ', models_to_predict_2$k[i], sep = ""))
   abline(1, 1, lty = 2)
   arrows(x0 = 60, y0 = 70, x1 = 65, y1 = 70, length = 0.1, angle = 30, code = 2)
   text(x = 45, y = 70, labels = "Line of 1:1 agreement", cex = .75)
   # model_var = sum((predicted_recapture_lengths[i, ] - tagdat_validate[ ,2])^2) / length(tagdat_validate[ ,2])
 ...
 ### Figure 1: Stat Grid Map
 ## Plotting Interpolated bathymetry
 library('marmap')
 library('doParallel')
 library('beepr')
 ...
 bty = raster('/Volumes/GoogleDrive/My Drive/Weng Lab/Personal_Folders/Steve/dissertation work/Ch 5. BACIP Analysis/50m Bathy Resampled from Mosaic.gri')
 stat_grids = readOGR(file.path('/Volumes/GoogleDrive/My Drive/Weng Lab/Personal_Folders/Steve/dissertation work/Ch 5. BACIP Analysis/', 'data/mhi_brfa_fishgrid_map.mpk Folder/commondata/regs/Fishchart2008.shp'))
 
 ### Stat grids to keep
 stat_grids_to_keep = c(100:800)
 stat_grids = stat_grids[stat_grids$AREA_ID %in% stat_grids_to_keep, ]
 
 # Remove bathymetry > 0
 bty[bty > 0] = NA
 
 ## Plot Figure
 pdf(file.path(results_dir, 'Figure 1a. Bathy.pdf'), height = 55, width = 66)
 ## Ploty bty
 plot(bty, col = 'lightblue', ylim = c(17, 24), xlim = c(-162, -153.5))
 
 ## Plot stat grids
 # plot(stat_grids, border = 'darkgrey', add = T)
 # text(stat_grids, labels = stat_grids$AREA_ID, col = 'black', cex = 3)
 
 dev.off()
 
 ## Ploty Stat Grids Only
 pdf(file.path(results_dir, 'Figure 1b. Stat Grids.pdf'), height = 55, width = 66)
 plot(bty, col = 'white', ylim = c(17, 24), xlim = c(-162, -153.5))
  plot(stat_grids, col = 'lightblue', border = 'darkgrey', add = T)
 # text(stat_grids, labels = stat_grids$AREA_ID, col = 'black', cex = 3)
 
 dev.off()
 
 pdf(file.path(results_dir, 'Figure 1c. Stat Grid Labels.pdf'), height = 55, width = 66)
 
 ## Ploty Stat Grid Labels
 plot(bty, col = 'lighblue', ylim = c(17, 24), xlim = c(-162, -153.5))
