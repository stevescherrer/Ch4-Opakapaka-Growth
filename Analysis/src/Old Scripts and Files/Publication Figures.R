...
proj_dir = '/Users/stephenscherrer/Google Drive/Weng Lab/Data/Bottomfish/Okamoto_Mark_Recapture'
...
legend('topright', c("Tagging", "Recapture"), fill = c(rgb(1,0,0,0.5), rgb(0,0,1,0.5)))
...
pdf(file.path(results_dir, 'Figure 5 - VBGF Plots for Bayesian and MLE models.pdf'), height = 8.5, width = 11)
max_age = 20
par(mfrow = c(1, 1))
plot(x = c(0, max_age), y = c(0, 100), type = "n",
...
for(i in 1:length(models_to_predict$author)){
  ...
  lines(estimates ~ seq(0, max_age, .25), col = plot_col[i])
}

abline(min(tagdat[ ,1]), 0, lty = 2)
abline(max(tagdat[ ,1]), 0, lty = 2)
text(x = .5, y = min(tagdat[ ,1]) + 5, labels = 'min Lm')
text(x = .5, y = max(tagdat[ ,1]) + 5, labels = 'max Lm')

legend('bottomright', legend = row.names(predicted_recapture_lengths), lty = c(1, 1, 1, 1, 1, 1, 1, 1, 2), col = c(plot_col, 'black', 'black'), cex = 1)
dev.off()

### For all studies
pdf(file.path(results_dir, 'VBGF Plots for all studies.pdf'), height = 8.5, width = 11)
plot(x = c(0, 46), y = c(0, 100), type = "n",
     xlim = c(0, 46), xlab = 'Age (years)',
     ylim = c(0, 100), ylab = "Length (cm)",
     main = "VBGF for P. Filamentosus")
